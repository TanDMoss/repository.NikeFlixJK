...
class Player(xbmc.Player):
    def __init__(self):
        ...
        self.info = {}
        self.path = None
        self.type = 'unknown'  # Initialized here
    ...
    def onPlayBackEnded(self):
        ...
        self.info = {}
        content_type = getattr(self, 'type', 'unknown')  # Safeguard here
        cache.save_playback_history(content_type, pp, self.path)
        utils.log("recorded playback of {}% {}".format(pp, content_type), "notice")

        for hash, path in cache.widgets_changed_by_watching(content_type):
            cache.push_cache_queue(path)
        utils.log("++ Finished queuing widget updates after playback ++", "notice")
